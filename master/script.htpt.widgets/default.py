...
class Main:
        ...
        if not xbmc.abortRequested:
            ...
            if json_query.has_key('result') and json_query['result'].has_key('movies'):
                ...
                for item in json_query['result']['movies']:
                    ...
                    self.WINDOW.setProperty("%s.%d.Year"            % (request, count), str(year))
    ...
    def _fetch_tvshows_recommended(self, request):
        if not xbmc.abortRequested:
            ...
            json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": {"properties": ["title", "studio", "mpaa", "file", "art", "year", "genre"], "sort": {"order": "descending", "method": "lastplayed"}, "filter": {"field": "inprogress", "operator": "true", "value": ""}, "limits": {"end": %d}}, "id": 1}' %self.LIMIT)
            json_query = unicode(json_query, 'utf-8', errors='ignore')
            json_query = simplejson.loads(json_query)
        ...
        if not xbmc.abortRequested:
            ...
            if json_query.has_key('result') and json_query['result'].has_key('episodes'):
                ...
                for item in json_query['result']['episodes']:
                    ...
                    json_query2 = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShowDetails", "params": {"properties": ["file", "studio", "genre", "year"], "tvshowid":%s}, "id": 1}' %item['tvshowid'])
                    json_query2 = unicode(json_query2, 'utf-8', errors='ignore')
                    json_query2 = simplejson.loads(json_query2)
                    ...
                    #year = str(item['year'])
                    if admin:
                        print "json_query: " + str(json_query)
                        print "json_query2: " + str(json_query2)
                        print "test_widget" + " " + str(genre) + "\n" + "year: " + str(year)
                    ...
                    self.WINDOW.setProperty("%s.%d.Year"                % (request, count), str(year))
                    self.WINDOW.setProperty("%s.%d.Genre"               % (request, count), " / ".join(genre))
                    self.WINDOW.setProperty("%s.%d.Resume"              % (request, count), resume)
                    self.WINDOW.setProperty("%s.%d.PercentPlayed"       % (request, count), played)
                    self.WINDOW.setProperty("%s.%d.Watched"             % (request, count), watched)
                    self.WINDOW.setProperty("%s.%d.File"                % (request, count), item['file'])
