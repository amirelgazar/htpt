...
def addonsettings(addon,skinsetting,set1,set2,set3,opt1,opt2,opt3):
	'''------------------------------
	...
	if skinsetting != "":
		setting_custom = xbmc.getInfoLabel(skinsetting)
		if setting_custom == truestr: setting_custom = "true"
		else:
			setting_custom = "false"
	elif skinsetting == "": setting_custom = ""
	
	''' set1 = username, set2 = password '''
	...
	if (setting_custom == "true" or id9str == 'Trial' or id2str == datenowS) and set1 != "" and set2 != "":
		if admin: notification("test0","","",1000)
		if ('htpt' in setting1 and not "finalmakerr" in idstr) and idstr != setting1 and id9str != 'Trial' and id2str != datenowS:
			setsetting_custom(set1,idstr)
			setsetting_custom(set2,idpstr)
			printpoint = printpoint + "1"
			if admin: notification("test1","","",1000)
		elif id9str == 'Trial' or id2str == datenowS:
			setsetting_custom(set1,idtrial)
			setsetting_custom(set2,idp2str)
			printpoint = printpoint + "2"
			if admin: notification("test2","","",1000)
		elif setting_custom == "true" and (setting1 == "" or setting2 == ""):
			setsetting_custom(set1,"")
			setsetting_custom(set2,"")
			printpoint = printpoint + "1"
		...
		if "RealDebrid" in skinsetting and (daynowS == opt1 or General_TimeZone != opt2):
			'''exception'''
			setsetting_custom(set1,"")
			setsetting_custom(set2,"")
	...
	---PRINT-END---------------------
	------------------------------'''
	if printpoint == "0": print printfirst + "addonsettings LV_0" + space2 + addon + space + skinsetting + space2 + setting_custom + space + "?"
	if printpoint == "1": print printfirst + "addonsettings LV_1" + space2 + addon + space + skinsetting + space2 + setting_custom + space + "RESET"
	elif printpoint == "2": print printfirst + "addonsettings LV_2" + space2 + addon + space + skinsetting + space2 + setting_custom + space + "Trial / DATENOW"
	elif printpoint == "3": print printfirst + "addonsettings LV_3" + space2 + addon + space + skinsetting + space2 + setting_custom + space + "NONE" + space + "ID: " + idstr + space + "daynowS" + space2 + daynowS + space + "timenow3S" + space2 + timenow3S + space + "datenowS" + space2 + datenowS + space + "timezone" + space2 + timezone
	elif printpoint == "4": print printfirst + "addonsettings LV_4" + space2 + addon + space + skinsetting + space2 + setting_custom + space + "DEFAULT" + space + "ID: " + idstr + space + "timezone" + space2 + timezone
...
def setGeneral_TimeZone(General_ScriptON):
	...
	if General_TimeZone != timezone: print printfirst + "setGeneral_TimeZone" + space2 + General_TimeZone + " - " + General_TimeZone2 + " (" + timezone + " )" + space3
		...
		print printfirst + "setGeneral_TimeZone" + space2 + General_TimeZone + " (" + timezone + " )" + space3
...
def setGeneral_Refresh(General_ScriptON):
	...
	print printfirst + "setGeneral_Refresh" + space2 + General_Refresh + " - " + General_Refresh2 + space3
	'''---------------------------'''
...
def setGeneral_StartWindow(General_ScriptON, General_StartWindow):
	...
	print printfirst + "setGeneral_StartWindow" + space2 + General_StartWindow + " - " + General_StartWindow2 + space3 + "General_ScriptON" + space2 + General_ScriptON + space3 + "myvideonavW/homeW" + space2 + myvideonavW + "/" + homeW + space3
...
def setCurrent_M_T(General_StartWindow):
	...
	---PRINT-END---------------------
	------------------------------'''
	print printfirst + "setCurrent_M_T" + space2 + "ismovieS" + space2 + ismovieS + space + "istvS" + space2 + istvS + space + "istv4S" + space2 + istv4S + space + "ismovie4S" + space2 + ismovie4S + space + "General_StartWindow" + space2 + General_StartWindow + space3
	'''---------------------------'''
...
def topvideoinformation(General_StartWindow,Current_M_T):
	...
	if Current_M_T == "0":
		...
		if topvideoinformation8a != topvideoinformation8:
			...
			---PRINT-END---------------------
			------------------------------'''
			print printfirst + "topvideoinformation" + space2 + "clear TopVideoInformation"
			'''---------------------------'''
def setCurrent_Name(Current_Name2,Current_Source2,Current_M_T2,General_StartWindow):
	...
	---PRINT-END---------------------
	------------------------------'''
	print printfirst + "setCurrent_Name" + space2 + "Current_Name" + space2 + Current_Name + space3 + "Current_Name2" + space2 + Current_Name2 + space + "Current_M_T/2" + space2 + Current_M_T + " / " + Current_M_T2
	'''---------------------------'''
...
def setGeneral_Connected(connectedv2):
	...
	---VARIABLES---------------------
	------------------------------'''
	from variables import admin, systemplatformwindows, getsetting, setsetting
...
def setGeneral_CustomVAR(General_CustomVAR, General_ScriptON, Current_Dialog2):
	...
	if General_ScriptON == "false":
		...
		if networkgatewayaddress == "" or networkipaddress == "":
			...
			if Current_Dialog2 == "dialogselectW" and Current_Dialog != "dialogokW" and Current_Source != "": 
				'''------------------------------
				---SOURCES-NOT-FOUND-(4)---------
				------------------------------'''
				...
				setsetting_custom1('script.htpt.refresh','General_CustomVAR',"...")
				'''---------------------------'''
			elif (General_CountWaitN > 5 and General_CountWaitN < 15):
			...
			elif General_CountWaitN > 15 and General_CustomVAR != "3":
				'''------------------------------
				---SLOW-SERVER---(3/5)-----------
				------------------------------'''
				if General_CountWaitN == 20 and (General_ConnectionScoreN < 2 or admin): notification(addonString(239),addonString(248),"",5000)
...
def setGeneral_ConnectionScore(Current_Dialog2,smooth):
	'''------------------------------
	---VARIABLES---------------------
	------------------------------'''
	from variables import admin
	...
	if not xbmc.Player().isPlayingVideo():
		if Current_Dialog == "dialogokW":
			...
			if General_ConnectionScoreN > 0:
				if General_CustomVAR != "1" and General_CustomVAR != "2" and General_CustomVAR != "4" and General_CustomVAR != "20":
					...
					---REDUCE------------------------
					------------------------------'''
					if AutoPlay_SD or AutoPlay_HD and not AutoPlay_Pause: calculate('script.htpt.refresh','General_ConnectionScore',"2")
					elif General_CustomVAR == "5" or General_CustomVAR == "3": calculate('script.htpt.refresh','General_ConnectionScore',"2")
		...
		if smooth == "false":
			...
			---REDUCE-VIDEO------------------
			------------------------------'''
			if General_ConnectionScoreN > 0: calculate('script.htpt.refresh','General_ConnectionScore',"2")
			
			General_ConnectionScore2 = getsetting('General_ConnectionScore')
			General_ConnectionScoreN = int(General_ConnectionScore2)
			'''---------------------------'''
			...
			---INCREASE---------------------
			------------------------------'''
			if General_ConnectionScoreN < 5: calculate('script.htpt.refresh','General_ConnectionScore',"1")
			
			General_ConnectionScore2 = getsetting('General_ConnectionScore')
			General_ConnectionScore2N = int(General_ConnectionScore)
			'''---------------------------'''
	
	#setSkinSetting("0",'General_ConnectionScore',General_ConnectionScore2)
	if General_ConnectionScore != General_ConnectionScore2: print printfirst + "setGeneral_ConnectionScore" + space2 + General_ConnectionScore + " - " + General_ConnectionScore2 + space + Current_Dialog + space3
...
def openDialogOK(Current_Dialog2):
	...
	------------------------------'''
	from variables import addonString, getsetting, setsetting
	from variables import Current_Name, Current_Source, General_Connected, General_CountWait
	connected = xbmc.getInfoLabel('Skin.HasSetting(Connected)')
	connected2 = xbmc.getInfoLabel('Skin.HasSetting(Connected2)')
	connected3 = xbmc.getInfoLabel('Skin.HasSetting(Connected3)')
	AutoPlay_Pause = getsetting('AutoPlay_Pause')
	...
	if General_CustomVAR == "1":
		...
		elif Current_Source == "" and General_ConnectionScoreN < 3: dialogok(addonString(229),addonString(231),"","")
		'''---------------------------'''
	...
	---PRINT-END---------------------
	------------------------------'''
	print printfirst + "openDialogOK" + space2 + "General_CustomVAR" + space2 + General_CustomVAR + space3
	'''---------------------------'''
...
def RefreshSettings():
	...
	#if not playerhasvideo: setsetting_custom1('script.htpt.refresh','General_StartWindow',"")
	'''------------------------------
	---General_ScriptON--------------
	------------------------------'''
	General_ScriptON = getsetting('General_ScriptON')
	...
	#AutoPlay_SD = getsetting('AutoPlay_SD')
	#AutoPlay_HD = getsetting('AutoPlay_HD')
	#setSkinSetting("1",'AutoPlay_Pause',AutoPlay_Pause)
	#setSkinSetting("1",'AutoPlay_SD',AutoPlay_SD)
	#setSkinSetting("1",'AutoPlay_HD',AutoPlay_HD)
	#if AutoPlay_Pause == "true": addonsettings2('plugin.video.genesis','autoplay',"false",'autoplay_hd',AutoPlay_HD,'autoplay_library',"false",'',"",'',"")
	#elif AutoPlay_Pause != "true": addonsettings2('plugin.video.genesis','autoplay',"true",'autoplay_hd',AutoPlay_HD,'autoplay_library',"true",'',"",'',"")
	'''---------------------------'''
	...
	---PRINT-END---------------------
	------------------------------'''
	if admin: print printfirst + "RefreshSettings" + space2 + "AutoPlay_Pause" + space2 + AutoPlay_Pause + space3
	'''---------------------------'''
...
def GenesisSettings(custom):
	...
	------------------------------'''
	from variables import getsetting, setsetting
	from variables import autoplaysd
	General_ConnectionScore = getsetting('General_ConnectionScore')
	General_ScriptON = getsetting('General_ScriptON')
	AutoPlay_Pause = getsetting('AutoPlay_Pause')
	General_ConnectionScoreN = int(General_ConnectionScore)
	...
	if custom == "0":
		'''ACCOUNTS'''
		addonsettings('plugin.video.genesis','Skin.HasSetting(RealDebrid)','realdedrid_user','realdedrid_password',"","Sun","A","")
		...
		'''HOSTS PRIORITY SD'''
		addonsettings2('plugin.video.genesis','host1',"Ororo",'host2',"MovieTV",'host3',"iShared",'host4',"Streamin",'host5',"VK")
		...
		if General_ConnectionScore == "5":
			...
			if AutoPlay_Pause == "true":
				...
				addonsettings2('script.htpt.refresh','',"",'AutoPlay_HD',"true",'AutoPlay_SD',"false",'playback_info',"",'',"")
				printpoint = printpoint + "2"
				'''---------------------------'''	
			else:
				'''------------------------------
			...
			if AutoPlay_Pause == "true":
				...
				------------------------------'''
				addonsettings2('plugin.video.genesis','autoplay',"false",'autoplay_hd',"false",'autoplay_library',"false",'playback_info',"false",'',"")
				addonsettings2('script.htpt.refresh','',"",'AutoPlay_HD',"false",'AutoPlay_SD',"false",'',"",'',"")
				printpoint = printpoint + "6"
			else:
				'''------------------------------
				---AutoPlay_Pause-false----------
	...
	AutoPlay_SD2 = getsetting('AutoPlay_SD')
	AutoPlay_HD2 = getsetting('AutoPlay_HD')
	xbmc.sleep(500)
	setSkinSetting("1",'AutoPlay_Pause',AutoPlay_Pause2)
	setSkinSetting("1",'AutoPlay_SD',AutoPlay_SD2)
	setSkinSetting("1",'AutoPlay_HD',AutoPlay_HD2)
